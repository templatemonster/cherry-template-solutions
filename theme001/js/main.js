$(document).ready(function() {
    ...
    if ($(".header_cart .hidden").text().length > 0) {
        var e = $(".header_cart .hidden").html().replace(/[^\d.]/g, '');
        $(".header_cart .header_cart__quantity").html(e);
    }

    if ($(".header_cart .hidden").text().length == 0) {
        $(".header_cart .header_cart__quantity").html('0');
    }
...
$(document).mouseup(function (e) {
    if ($("#ModalContainer").has(e.target).length === 0){
        $("#ModalOverlay, #ModalContainer").hide();
    }
});
